...
class DemandEnvironment:
    ...
    def compute_nash_equilibrium(self) -> float:
        ...
        For symmetric duopoly with logit demand using iterative best response.
        """
        from scipy.optimize import root_scalar
        
        def nash_foc(p, rival_p):
            """First-order condition for Nash equilibrium: ∂π_i/∂p_i = 0"""
            # Compute own demand given rival price
            prices = np.array([p, rival_p])
            demands = self.compute_demands(prices)
            q_i = demands[0]
            
            # Logit demand derivative: ∂D_i/∂p_i = -(1/μ) * D_i * (1 - D_i)
            dq_dp = -(1/self.mu) * q_i * (1 - q_i)
            
            # FOC: marginal revenue = marginal cost
            # ∂π_i/∂p_i = q_i + (p_i - c) * ∂q_i/∂p_i = 0
            foc = q_i + (p - self.c) * dq_dp
            return foc
        
        # Iterative best response to find symmetric Nash
        p_guess = self.c + 2.0  # Initial guess
        for iteration in range(50):  # Maximum iterations
            try:
                # Solve best response given current rival price
                result = root_scalar(
                    nash_foc, 
                    args=(p_guess,),
                    bracket=[self.c + 0.01, self.c + 10.0],
                    xtol=1e-10, 
                    rtol=1e-10
                )
                p_new = result.root
